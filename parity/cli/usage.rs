...
macro_rules! usage {
	(
		...
		macro_rules! hydrate_rawargs_with_globals {
			($raw_args:ident, $matches_ident:ident) => {
				$(
					// use hydrate_flags! instead?
					$(
						$raw_args.$flag = $matches_ident.is_present(stringify!($flag));
					)*
					$( // 1
						$raw_args.$arg = return_if_parse_error!(if_option!(
							...
							THEN {
								if_option_vec!(
									$($arg_type_tt)+,
									THEN { values_t!($matches_ident, stringify!($arg), inner_option_vec_type!($($arg_type_tt)+)) }
									ELSE { value_t!($matches_ident, stringify!($arg), inner_option_type!($($arg_type_tt)+)) }
								)
							}
							ELSE {
								if_vec!(
									$($arg_type_tt)+,
									THEN { values_t!($matches_ident, stringify!($arg), inner_vec_type!($($arg_type_tt)+)) }
									ELSE { value_t!($matches_ident, stringify!($arg), $($arg_type_tt)+) }
								)
							}
						));
					)*
		...
		impl RawArgs {
			...
			pub fn parse<S: AsRef<str>>(command: &[S]) -> Result<Self, ClapError> {
				...
				hydrate_rawargs_with_globals!(raw_args, matches);
				...
				$(
					if let Some(submatches) = matches.subcommand_matches(&underscore_to_hyphen!(&stringify!($subc)[4..])) {
						...
						// Globals
						hydrate_rawargs_with_globals!(raw_args, submatches);
						// Subcommand flags
						$(
						...
						// Global arguments
						// @todo pareil pour flags...
						// $( // 2 - it can be the same code if s/submatches/matches; so can be generated by a macro inside
						// 	raw_args.$arg = return_if_parse_error!(if_option!(
						// 		$($arg_type_tt)+,
						// 		THEN {
						// 			if_option_vec!(
						...
						$(
							if let Some(subsubmatches) = submatches.subcommand_matches(&underscore_to_hyphen!(&stringify!($subc_subc)[stringify!($subc).len()+1..])) {
								...
								// Globals
								hydrate_rawargs_with_globals!(raw_args, subsubmatches);
